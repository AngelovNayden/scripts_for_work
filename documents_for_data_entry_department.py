...
def find_pages_by_text(reader, search_text):
    ...
    """
    pages = []
    bestel, oberStof, verbrauchliste = search_text
...
def process_pdfs_for_printing(folder_path):
    ...
    for file_name in os.listdir(folder_path):
        if file_name.endswith(".PDF"):
            ...
            pages_to_print = find_pages_by_text(
                reader, ["Bestellpositionen", "Oberstoff Klassifizierung", "Verbrauchsliste"])
    ...
    print("Временни файлове за принтирани копия са запазени за преглед.")
...
def print_multiple_pdf(pdf_path, copies, file_name):
...
for file_name in os.listdir(sciprt_folder_path):
    if file_name.endswith(".pdf"):
        ...
        print_multiple_pdf(temp_pdf_path, copies, file_name)
